...
fetch('/api/status').then(function(r){return r.json();}).then(function(d){
  ...
  if(d.apMode){
    ...
    if(!d.apPasswordSet){
      ...
      document.getElementById('ap-pw-status').textContent='No password — hotspot is open';
...
function apSetPassword(){
  ...
  if(pw.length>0&&pw.length<8){toast('Min. 8 characters','err');return;}
    ...
    .then(function(r){return r.json();}).then(function(d){
      if(d.status==='ok'){
        ...
        if(d.reconnect){
          ...
          document.getElementById('ap-pw-status').textContent='Password cleared — hotspot is open';
          document.getElementById('ap-pw-status').style.color='var(--danger,#ff4444)';
          document.getElementById('ap-pw-warning').style.display='';
