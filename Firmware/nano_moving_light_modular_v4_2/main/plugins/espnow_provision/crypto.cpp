...
size_t crypto_aes128_cbc_encrypt(const uint8_t* key16, const uint8_t* iv16,
                                  ...
                                  uint8_t* cipher) {
    // PKCS#7 pad into a local buffer (max 160 bytes: 32 SSID + 1 + 64 PW + up to 16 pad)
    ...
    uint8_t iv[16];
    memcpy(iv, iv16, 16);  // mbedtls modifies iv in place
...
size_t crypto_aes128_cbc_decrypt(const uint8_t* key16, const uint8_t* iv16,
    ...
    // Strip PKCS#7 padding and validate it
