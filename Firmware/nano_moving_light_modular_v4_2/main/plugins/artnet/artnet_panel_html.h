...
// Matches PC App artnet_panel.html exactly.
// ─────────────────────────────────────────────────────────────────
...
(function(){
  ...
  window.an_patchSelected = function(){
    ...
    ids.forEach(function(fixID, i){
      var a = addr + i * DMX_FP;
      if(a + DMX_FP - 1 > 512){ skipped++; return; }
      toPost.push({fixID:fixID, universe:uni, startAddr:a});
    });
    if(!toPost.length){ if(typeof toast==='function') toast('No room in universe '+uni+' \u2014 increase Addr or use next universe','err'); return; }
    var pending = toPost.length;
    toPost.forEach(function(p){
      fetch('/api/artnet/patch',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify(p)
      }).then(function(r){return r.json();}).then(function(){
        if(--pending===0){
          if(typeof toast==='function') toast('Patched '+toPost.length+' fixture(s)'+(skipped?' ('+skipped+' skipped \u2014 overflow)':''));
          an_gotoUniverse(uni);
          an_load();
        }
      });
  ...
  window.an_clearUniverse = function(){
    fetch('/api/artnet/patch/universe/'+an_curUni,{method:'DELETE'})
      .then(function(r){return r.json();}).then(function(d){
        if(typeof toast==='function') toast('Universe '+an_curUni+' cleared ('+(d.removed||0)+' patch(es))');
        an_load();
      });
  };
  ...
  window.an_clearAll = function(){
    ...
    fetch('/api/artnet/patch',{method:'DELETE'})
      .then(function(r){return r.json();}).then(function(d){
        if(typeof toast==='function') toast('All patches cleared ('+(d.removed||0)+')');
        an_load();
      });
  };
