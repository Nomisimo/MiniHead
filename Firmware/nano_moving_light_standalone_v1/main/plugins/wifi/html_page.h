...
// ── Embedded HTML UI (Standalone v1) ─────────────────────────────
...
<style>
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .main{display:grid;grid-template-columns:1fr;grid-template-areas:"light" "motion" "rainbow" "serial" "cues" "sequencer";}
  @media(min-width:900px){
    .main{grid-template-columns:2fr 1fr;grid-template-rows:auto auto auto auto;grid-template-areas:"cues light" "sequencer motion" ". rainbow" ". serial";min-height:calc(100vh - 49px);}
  }
  .area-light{grid-area:light;}.area-motion{grid-area:motion;}.area-rainbow{grid-area:rainbow;}
  .area-serial{grid-area:serial;}.area-cues{grid-area:cues;}.area-sequencer{grid-area:sequencer;}
...
function sendCurrent(){var v=getValues();var cmd='R:'+v.r+',G:'+v.g+',B:'+v.b+',W:'+v.w+',PAN:'+v.pan+',TILT:'+v.tilt;sendCommand(cmd);}
...
function editCue(id,curTargets){_editCueId=id;fetch('/api/fixtures').then(function(r){return r.json();}).catch(function(){return[];}).then(function(fixtures){var list=document.getElementById('cueEditList');var html='<label class="modal-check-row"><input type="checkbox" id="cue_all" '+(curTargets.indexOf(0)>=0?'checked':'')+'><span style="color:var(--accent)">ALL</span></label>';fixtures.forEach(function(f){if(f.id<=0)return;var chk=curTargets.indexOf(f.id)>=0;html+='<label class="modal-check-row"><input type="checkbox" class="cue-fix-cb" value="'+f.id+'" '+(chk?'checked':'')+'>'+f.id+' &#8212; '+escHtml(f.name||'?')+'</label>';});list.innerHTML=html;var poolIds=fixtures.map(function(f){return f.id;});var free=curTargets.filter(function(id){return id>0&&poolIds.indexOf(id)<0;});document.getElementById('cueEditFree').value=free.join(', ');document.getElementById('cueEditModal').classList.add('open');});}
