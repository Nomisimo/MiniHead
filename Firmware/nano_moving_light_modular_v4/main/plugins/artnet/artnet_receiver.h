...
void artnet_receiver_loop() {
  ...
  // ── Red blink if Art-Net is active but this node has no patch ────
  // Condition: packets arriving, we have a FixID, but no patch entry for us.
  static unsigned long _noPatchBlinkLast = 0;
  static bool _noPatchBlinkState = false;
  if (artnetActive && ownFixID > 0) {
    bool ownPatched = false;
    for (int i = 0; i < artnetPatchCount; i++)
      if (artnetPatches[i].fixID == ownFixID) { ownPatched = true; break; }
    if (!ownPatched) {
      unsigned long now = millis();
      if (now - _noPatchBlinkLast > 250) {
        _noPatchBlinkLast = now;
        _noPatchBlinkState = !_noPatchBlinkState;
        setLED(_noPatchBlinkState ? 255 : 0, 0, 0, 0);
      }
    } else {
      _noPatchBlinkState = false;  // reset so blink stops immediately when patch arrives
    }
  }
  ...
  if (artnetActive && (millis() - artnetLastPacket > ARTNET_TIMEOUT_MS)) {
    artnetActive = false;
    _noPatchBlinkState = false;
    setLED(0, 0, 0, 0);  // ensure LED off after timeout
